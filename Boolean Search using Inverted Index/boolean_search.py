...
def tokenize(document):
    ...
    """
    ###TODO
    s=re.sub('\W+',' ', document.lower()).split()
    return s
    pass
...
def create_index(tokens):
    ...
    for i in range(0,len(tokens)): 
       for j in range(0,len(tokens[i])):
           if(tokens[i][j] in keydict):
               keydict[tokens[i][j]].append(i)
           else:
               keydict[tokens[i][j]]=[i]
    return keydict    
    pass
...
def sort_by_num_postings(words, index):
    ...
    """
    ###TODO
    return sorted(index,key=lambda x:len(index[x]))
...
def search(index, query):
    ...
    if(len(val)==1):
        ...
        for i in range(1,len(val)):
           output=intersect(output,index[val[i]])  
        return output        
    pass
